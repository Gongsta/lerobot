...
class GeminiProvider(LLMProvider):
    ...
    def format_messages(
        self, messages: list[dict[str, Any]], thinking_enabled: bool = False
    ) -> list[dict[str, Any]]:
        ...
        for msg in messages:
            ...
            if role == "tool":
                if isinstance(content, list):
                                ...
                                if source.get("type") == "base64":
                                    parts.append(
                                        types.Part.from_bytes(
                                            data=source["data"], mime_type=source["media_type"]
                                        )
                                    )
                        else:
                    ...
                    if parts:
                        formatted_messages.append(types.Content(role="function", parts=parts))
                else:
                    ...
                    formatted_messages.append(
                        types.Content(
                            ...
                            parts=[
                                types.Part.from_function_response(
                                    name="unknown", response={"result": content}
                                )
                            ],
                        )
                    )
            ...
            if isinstance(content, str):
                formatted_messages.append(
                    types.Content(
                        role="user" if role == "user" else "model", parts=[types.Part(text=content)]
                    )
                )
            else:
                ...
                for part in content:
                    if isinstance(part, dict):
                        if part.get("type") == "text":
                            ...
                            if source.get("type") == "base64":
                                parts.append(
                                    types.Part.from_bytes(data=source["data"], mime_type=source["media_type"])
                                )
                    else:
                        parts.append(types.Part(text=str(part)))

                formatted_messages.append(
                    types.Content(role="user" if role == "user" else "model", parts=parts)
                )
    ...
    def format_tool_results_for_conversation(
        self, tool_calls: list[dict[str, Any]], tool_outputs: list[list[dict[str, Any]]]
    ) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        """Format tool results for Gemini conversation history."""
        tool_results_with_images = []
        image_parts = []

        for tool_call, tool_output_parts in zip(tool_calls, tool_outputs, strict=False):
            text_parts = []
            current_image_parts = []
            ...
            for i, image_part in enumerate(current_image_parts, 1):
                tool_results_with_images.append(
                    {"type": "text", "text": f"Image {i} from {tool_call['name']}:"}
                )
    ...
    async def _generate_response_impl(
        ...
        stream = self.client.models.generate_content_stream(
            model=self.model, contents=formatted_messages, config=config
        )
