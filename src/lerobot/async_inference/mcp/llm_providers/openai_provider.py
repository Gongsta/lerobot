...
class OpenAIProvider(LLMProvider):
            ...
            if role == "system":
                ...
                if self._is_reasoning_model() and system_content and len(formatted_messages) == 0:
                    if isinstance(formatted_message["content"], str):
                        ...
                        formatted_message["content"].insert(
                ...
                if "tool_calls" in message and message["tool_calls"]:
                    formatted_message = {"role": "assistant", "content": content or None}
                ...
                if isinstance(content, list):
                    ...
                    tool_messages.append({"role": "tool", "tool_call_id": "unknown", "content": str(content)})
    ...
    def format_tool_results_for_conversation(
        ...
        for tool_call, output_parts in zip(tool_calls, tool_outputs, strict=False):
    ...
    def _format_message(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        if role == "tool":
            ...
            if isinstance(content, list):
                ...
                return {"role": "tool", "tool_call_id": "unknown", "content": str(content)}
            else:
    ...
    async def _generate_response_impl(
        ...
        return LLMResponse(
            content=full_content or None, tool_calls=tool_calls if tool_calls else None, usage=final_usage
        )
