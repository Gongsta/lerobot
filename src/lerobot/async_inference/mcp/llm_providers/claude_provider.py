...
class ClaudeProvider(LLMProvider):
    ...
    def __init__(self, api_key: str | None = None, model: str = "claude-3-7-sonnet-latest"):
        if not api_key:
                ...
                # If thinking is disabled, convert the thinking block to simple text content
                    ...
                    if isinstance(thinking_block, dict) and "thinking" in thinking_block:
                        assistant_content.append(
                            {"type": "text", "text": f"I previously thought: {thinking_block['thinking']}"}
                        )
    ...
    async def _generate_response_impl(
        ...
        with self.client.messages.stream(**stream_params) as stream:
            ...
            for event in stream:
                ...
                elif event.type == "content_block_stop" and (thinking_started or response_started):
