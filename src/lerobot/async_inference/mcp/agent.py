...
from llm_providers.factory import create_llm_provider  # noqa: E402
...
class AIAgent:
    ...
    def __init__(
        ...
        mcp_server_ip: str = "127.0.0.1",
        mcp_port: int = 3001,
        thinking_budget: int = 1024,
        thinking_every_n: int = 1,
        api_key: str = None,
    ):
        self.model = model
                ...
                if filtered_content:
                    if len(filtered_content) == 1 and filtered_content[0].get("type") == "text":
                        filtered_conversation.append(
                            {"role": msg["role"], "content": filtered_content[0].get("text", "")}
                        )
                    else:
                        filtered_conversation.append({"role": msg["role"], "content": filtered_content})
            else:
    ...
    async def process_with_llm(self, user_input: str) -> str:
        ...
        for iteration in range(max_iterations):
            try:
                ...
                tool_calls = self.llm_provider.format_tool_calls_for_execution(
                ...
                tool_results_with_images, image_parts = (
                    self.llm_provider.format_tool_results_for_conversation(tool_calls, tool_outputs)
                )
                ...
                if image_parts:
                    self.conversation_history = self._filter_images_from_conversation(
                        self.conversation_history
                    )
...
async def main():
    ...
    parser.add_argument("--api-key", help="API key for the selected model (overrides env vars)")
    parser.add_argument(
        "--model",
        default="claude-3-7-sonnet-latest",
        help="Model to use (claude-3-7-sonnet-latest, gemini-2.5-flash, etc.)",
    )
    parser.add_argument("--show-images", action="store_true", help="Show images in window")
    parser.add_argument(
        "--mcp-server-ip",
    ...
    parser.add_argument("--thinking-budget", type=int, default=1024, help="Thinking budget in tokens")
